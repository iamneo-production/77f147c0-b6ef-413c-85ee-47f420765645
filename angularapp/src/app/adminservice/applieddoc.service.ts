...
export class ApplieddocService {
  ...
  getProfile(id:number) {
    console.log(id);
    return this.httpclient.get<Applieddoc>(`https://8080-faafcafbddfacbdcaababfebeecbbaefdfd.project.examly.io/getProfile/${id}`); // Replace '1' with the actual user ID
  }
  approveProfile(id: number): Observable<Applieddoc> {
    const url = `https://8080-faafcafbddfacbdcaababfebeecbbaefdfd.project.examly.io/getProfile/${id}/approve`;
    return this.httpclient.put<Applieddoc>(url, null);
  }
  rejectProfile(id: number): Observable<Applieddoc> {
    const url = `https://8080-faafcafbddfacbdcaababfebeecbbaefdfd.project.examly.io/getProfile/${id}/reject`;
    return this.httpclient.put<Applieddoc>(url, null);
  }
  downloadImageByProfileId(id: number): Observable<HttpResponse<Blob>> {
    const url = `https://8080-faafcafbddfacbdcaababfebeecbbaefdfd.project.examly.io/getDocuments/profile/${id}`;
